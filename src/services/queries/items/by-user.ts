...
export const itemsByUser = async (userId: string, opts: QueryOpts) => {
	const query = `@ownerId:{${userId}}`; // when search by tags, use {tag}
	...
	console.log(total, documents, 'total, documents');

	const items = documents.map(({ id, value }) => deserialize(id, value as any));
